...
import { selectAllAdvancedCourses, selectAllBeginnerCourses, selectPromoTotal } from '../courses.selector';
...
export class HomeComponent implements OnInit {
  ...
  reload() {
    ...
    this.beginnerCourses$ = this.store.pipe(
      select(selectAllBeginnerCourses)
    )
    this.advancedCourses$ = this.store.pipe(
      select(selectAllAdvancedCourses)
    )
